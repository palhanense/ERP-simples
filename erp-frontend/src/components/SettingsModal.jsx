...
export default function SettingsModal({ isOpen, onClose, isDark, setIsDark, storeName, setStoreName }) {
  ...
  const handleSave = () => {
    try {
      ...
      // keep compatibility with older key
      window.localStorage.setItem('erp-client-name', localName || '');
    } catch (e) {
      // ignore localStorage errors silently
    }
    setStoreName && setStoreName(localName || '');
    onClose && onClose();
  };
      ...
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-lg dark:bg-surface-dark">
        ...
        <div className="mt-4 space-y-4 text-sm text-neutral-700 dark:text-neutral-300">
          <div>
            ...
            <input value={localName} onChange={(e) => setLocalName(e.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2" placeholder="Nome da loja (opcional)" />
          </div>

          <div>
            <label className="block text-sm text-neutral-600">Tema</label>
            <div className="mt-2 flex items-center gap-3">
              <button type="button" onClick={() => setIsDark(false)} className={`rounded-full border px-3 py-2 ${!isDark ? 'bg-black text-white' : ''}`}>Dia</button>
              <button type="button" onClick={() => setIsDark(true)} className={`rounded-full border px-3 py-2 ${isDark ? 'bg-black text-white' : ''}`}>Noite</button>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-between items-center gap-3">
          <div className="flex items-center gap-3">
            <button onClick={() => { signOut(); onClose && onClose(); }} className="rounded-full border px-4 py-2 text-sm text-red-600">Sair</button>
          </div>
          <div className="flex items-center gap-3">
